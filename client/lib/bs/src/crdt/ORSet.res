...
// Tag comparison for Set - must be defined before type t
...
type t = {
  elements: Belt.Map.String.t<Belt.Set.t<tag, TagCmp.identity>>,
  version: int,
}
...
let merge = (a: t, b: t): t => {
...

  {
    ...
    version: Math.Int.max(a.version, b.version) + 1,
  }
}
...
type wireFormat = {
  ...
  elements: dict<array<tagWire>>,
  version: int,
  value: array<string>,
}

let toWire = (set: t): wireFormat => {
  let elementsDict = Dict.make()
  set.elements->Belt.Map.String.forEach((element, tags) => {
    ...
    Dict.set(elementsDict, element, tagsArray)
  })
  {
    type_: "or_set",
