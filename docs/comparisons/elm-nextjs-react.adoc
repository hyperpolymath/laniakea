...
This document provides an exhaustive comparison of Elm, Next.js, and
React — and explains why Laniakea takes a fundamentally different
approach that transcends all three.

=== Executive Summary

[width="100%",cols="21%,11%,16%,20%,32%",options="header",]
|===
|Aspect |Elm |React |Next.js |*Laniakea*
|*Type* |Functional language |UI library |React framework |Distributed
architecture

|*Philosophy* |Correctness via types |Components + hooks |Full-stack
React |Browser as BEAM peer

|*State* |Centralized (Model) |Component/global |Server + client |CRDT
convergence
...
[width="100%",cols="28%,17%,24%,31%",options="header",]
|===
|Aspect |Elm |React |Next.js
|*Type* |Functional programming language |JavaScript UI library
|Full-stack React framework

|*Created by* |Evan Czaplicki (2012) |Meta/Facebook (2013) |Vercel
(2016)

|*License* |BSD-3-Clause |MIT |MIT

|*Primary goal* |No runtime exceptions |Declarative UI components
...
|*Type system* |Hindley-Milner (sound) |Optional (TypeScript) |Optional
(TypeScript)
...
*Elm guarantees no runtime exceptions in pure Elm code; JS interop can
introduce them.
...
|===
|Aspect |Elm |React |Next.js
|*Architecture pattern* |The Elm Architecture (TEA) |Component-based
|Pages + App Router

|*State management* |Single Model |useState/useReducer/Redux/etc. |React
state + Server state

|*Data flow* |Unidirectional |Unidirectional (recommended)
|Bidirectional (Server Actions)

|*Side effects* |Cmd/Sub (managed) |useEffect (unmanaged) |useEffect +
Server Actions
...
|*Error messages* |Exceptional (famous for quality) |Variable (depends
on tooling) |Variable

|*Debugging* |Time-travel debugger |React DevTools |React DevTools +
Next.js tools
...
|*Build tooling* |elm make (simple) |Webpack/Vite/etc. |Built-in
(Turbopack)
|===
...
|===
|Aspect |Elm |React |Next.js
|*Bundle size* |Small (~29KB gzipped) |~42KB (React + ReactDOM) |Varies
(includes React)
...
|*UI libraries* |elm-ui, elm-css |Material-UI, Chakra, etc. |All React
libraries
...
|*Real-time updates* |Manual implementation |Manual implementation
|Manual implementation

|*Offline support* |Manual (Service Workers) |Manual (Service Workers)
|Manual (Service Workers)

|*State sync* |Not built-in |Not built-in |Not built-in

|*Optimistic updates* |Manual |Manual (TanStack Query, SWR) |Server
Actions (partial)
|===
...
|===
|Aspect |Elm |React |Next.js
|*Server rendering* |Limited (elm-pages) |Via frameworks |Native
SSR/SSG/ISR

|*API routes* |None (frontend only) |None (frontend only) |Built-in

|*Database access* |Via API calls |Via API calls |Direct (Server
Components)
...
|===
|Aspect |Elm |React |Next.js
|*Approach* |Maybe/Result types |try/catch + Error Boundaries |try/catch
+ Error Boundaries
...
|*Partial failures* |Type-safe handling |Manual handling |Manual
handling
|===
...
The comparison above highlights a fundamental limitation shared by Elm,
React, and Next.js:

*They all treat the browser as a client that requests state from a
server.*
...
You could try to bolt CRDTs and offline support onto Elm, React, or
Next.js:
...
Laniakea is designed from the ground up for distributed state
convergence. The architecture assumes:

[arabic]
. State exists on multiple nodes
. Nodes can be offline
. State will eventually converge
. Neither side is authoritative

This is fundamentally different from "`add offline support to a
client-server app.`"
...
*Pros*: Predictable, debuggable, type-safe *Cons*: All state is local;
no built-in sync
...
*Pros*: Familiar, flexible *Cons*: Side effects can be anywhere; no sync
...
*Pros*: Server state automatically synced *Cons*: Requires server; no
offline; optimistic updates are manual
...
Elm, React, and Next.js are all excellent tools for their intended
purposes:
...
Laniakea challenges this assumption. By treating the browser as a peer
node in a distributed system, using CRDTs for conflict-free state
convergence, and building on BEAM’s proven distributed systems
foundation, Laniakea enables a new class of applications:
...
The future isn’t about picking the best client framework. It’s about
recognizing that the browser is a capable compute node that deserves to
participate in distributed systems as a first-class citizen.

'''''

=== Further Reading

* https://martin.kleppmann.com/2020/07/06/crdt-hard-parts-hydra.html[CRDTs:
The Hard Parts] — Martin Kleppmann
* https://www.inkandswitch.com/local-first/[Local-First Software] — Ink
& Switch
* https://guide.elm-lang.org/architecture/[The Elm Architecture] — Elm
Guide
* https://www.phoenixframework.org/[Why Phoenix] — Phoenix Framework
* https://popcorn.swmansion.com/[Popcorn: Elixir in the Browser] —
Software Mansion
